...
local themeCallbackRegistered = false

function Showcase.load(pixelFont)
    font = pixelFont
    Showcase.build()
    if not themeCallbackRegistered then
        themeCallbackRegistered = true
        Theme.onChange(function(theme)
            if Showcase._active then
                love.graphics.setBackgroundColor(theme.colors.background)
                Showcase.rebuild()
            end
        end)
    end
    Showcase._active = true
    love.graphics.setBackgroundColor(Theme.color("background"))
end

function Showcase.switchTab(index)
...
function Showcase.build()
    ...
    Showcase.buildSettingsPanel()
    Showcase.buildStatsPanel()
    Showcase.buildJuicePanel()
...
function Showcase.buildSettingsPanel()
    ...
    diffDropdown.onChange = function(self, value)
        Showcase.showNotification("Difficulty: " .. value)
    end
    ...
    applyBtn.onClick = function()
        ...
        Showcase.showNotification("Settings applied!")
    end
    ...
    for i = 1, 12 do
        ...
        local hue = (i - 1) / 12
        local r, g, b = Showcase.hslToRgb(hue, 0.5, 0.35)
        itemBtn.colors = {
            normal = {r, g, b, 1},
        ...
        itemBtn.onClick = function()
            Showcase.showNotification("Selected: Item " .. i)
        end
    end
end

function Showcase.buildStatsPanel()
    local panel = KuruUI.add(KuruUI.Panel:new(30, 80, 740, 440))
    Theme.applyToElement(panel, "panel")
    table.insert(tabPanels, panel)
    ...
    castBtn.onClick = function()
        if elements.mpBar:getValue() >= 10 then
            ...
            Showcase.showNotification("Spell cast!")
        else
            Juice.shake(castBtn, 6, 0.3)
            Showcase.showNotification("Not enough mana!")
        end
    end
    ...
    restBtn.onClick = function()
        ...
        Showcase.showNotification("Resting...")
    end
end

function Showcase.buildJuicePanel()
    local panel = KuruUI.add(KuruUI.Panel:new(30, 80, 740, 440))
    Theme.applyToElement(panel, "panel")
    table.insert(tabPanels, panel)
    ...
    typeBtn.onClick = function()
        Juice.typewriter(elements.typeLabel, "Hello! This is a typewriter effect...", 0.05, nil, function()
            Showcase.showNotification("Done typing!")
        end)
    end
...
function Showcase.rebuild()
    ...
    Showcase.switchTab(savedTab)
end

function Showcase.hslToRgb(h, s, l)
    local r, g, b
    if s == 0 then
        r, g, b = l, l, l
    else
...
function Showcase.showNotification(text)
...
function Showcase.mousepressed(x, y, button)
    KuruUI.mousepressed(x, y, button)
end

function Showcase.mousereleased(x, y, button)
    KuruUI.mousereleased(x, y, button)
end

function Showcase.wheelmoved(x, y)
...
function Showcase.textinput(text)
    KuruUI.textinput(text)
end

function Showcase.unload()
    Showcase._active = false
    KuruUI.clear()
end
